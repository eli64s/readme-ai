...
> [!IMPORTANT]
>
> <em>README-AI</em> is currently under development with an opinionated configuration and setup.
>
> Its vital to review all text generated by the LLM APIs to ensure it accurately represents your project.

---

## 🤖 Demo

**CLI**: Standard usage of the <em>readme-ai</em> CLI tool with an OpenAI API key.
...
Use `nox` to run the test suite against multiple Python versions. Currently runs test agains versions `3.9, 3.10, 3.11, 3.12`.
```sh
nox -f noxfile.py
```
> [!NOTE]
...
Run the `readmeai` command in your terminal with the following options to tailor your README file generation process.

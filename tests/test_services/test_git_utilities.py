...
async def test_clone_invalid_repo():
    ...
    with tempfile.TemporaryDirectory() as temp_dir, pytest.raises(GitCloneError) as exc:
...
async def test_clone_non_git_url(tmp_path):
    ...
    with mock.patch("git.Repo.clone_from") as mock_clone, pytest.raises(
        ...
        mock_clone.side_effect = git.GitCommandError("clone", "error")
        await clone_repo_to_temp_dir("http://example.com/not-a-git-repo", tmp_path)
        assert isinstance(exc.value, GitCloneError)
...
def test_fetch_git_file_url():
    ...
    expected_url = "https://github.com/eli64s/readme-ai/blob/master/readmeai/main.py"
    assert file_url == expected_url


@pytest.mark.asyncio
async def test_fetch_git_api_url():
    ...
    assert await fetch_git_api_url(git_service_url) == expected_api_url
...
@mock.patch("readmeai.services.git_utilities.platform")

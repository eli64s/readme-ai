...
from readmeai.services.metadata import (
    RepositoryMetadata,
...
async def test_fetch_git_repository_metadata_success():
    ...
    with patch(
        "readmeai.services.git.fetch_git_api_url",
        return_value="api_url",
    ), patch(
        "readmeai.services.metadata._fetch_repository_metadata",
        return_value={
            "name": repo_name,
            "full_name": full_name,
        },
...
async def test_fetch_git_repository_metadata_failure():
    """Tests the fetch_git_repository_metadata method with an exception."""
    with patch(
        "readmeai.services.metadata._fetch_repository_metadata",
        side_effect=aiohttp.ClientError,
    ):
        with pytest.raises(Exception) as exc:
            await fetch_git_repository_metadata(AsyncMock(), None)
